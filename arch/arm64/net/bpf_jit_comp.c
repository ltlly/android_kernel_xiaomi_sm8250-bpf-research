...
emit_bswap_uxt:
	...
	case BPF_JMP32 | BPF_JSLE | BPF_X:
		emit(A64_CMP(BPF_CLASS(code) == BPF_JMP, dst, src), ctx);
emit_cond_jmp:
		jmp_offset = bpf2a64_offset(i + off, i, ctx);
	...
	case BPF_JMP32 | BPF_JSET | BPF_X:
		emit(A64_TST(BPF_CLASS(code) == BPF_JMP, dst, src), ctx);
		goto emit_cond_jmp;
	...
	case BPF_JMP32 | BPF_JSLE | BPF_K:
		emit_a64_mov_i(BPF_CLASS(code) == BPF_JMP, tmp, imm, ctx);
		emit(A64_CMP(BPF_CLASS(code) == BPF_JMP, dst, tmp), ctx);
		goto emit_cond_jmp;
	case BPF_JMP | BPF_JSET | BPF_K:
	case BPF_JMP32 | BPF_JSET | BPF_K:
		emit_a64_mov_i(BPF_CLASS(code) == BPF_JMP, tmp, imm, ctx);
		emit(A64_TST(BPF_CLASS(code) == BPF_JMP, dst, tmp), ctx);
		goto emit_cond_jmp;
	/* function call */
