...
/*
 * 4.19-research-fork: backport from upstream Linux 6.0+. Generates a
 * load/store with immediate unsigned offset (LDR/STR Xt, [Xn, #imm]).
 * Required by the BPF trampoline emitter to access stack slots.
 *
 * imm is the byte offset; it must be aligned to the access size and fit
 * in 12 bits after shifting (i.e. 0..32760 for 64-bit accesses).
 */
static const u32 ksu_aarch64_insn_ldst_size[] = {
				    ...
				    enum aarch64_insn_ldst_type type)
{
	...
	shift = ksu_aarch64_insn_ldst_size[size];
